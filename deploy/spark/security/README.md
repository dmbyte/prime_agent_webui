...
The OpenShell installer is idempotent for the supported steady state. Before any
manual restore, back up `/var/lib/prime-runner`, `~/.prime/agent`, user service
drop-ins, and the Nginx TLS/configuration tree; then restore only the intended
files, reload systemd/Nginx as appropriate, and verify listeners before
re-enabling any network service.
