...
exec docker run -d --restart unless-stopped \
  ...
  --max-model-len "${MAX_MODEL_LEN:-81920}" --gpu-memory-utilization "${GPU_MEMORY_UTILIZATION:-0.38}" \
  --kv-cache-memory-bytes "${KV_CACHE_MEMORY_BYTES:-4G}" --max-num-seqs "${MAX_NUM_SEQS:-2}" \
