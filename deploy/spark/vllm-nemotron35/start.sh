...
exec docker run -d --restart unless-stopped \
  ...
  --max-model-len "${MAX_MODEL_LEN:-65536}" --gpu-memory-utilization "${GPU_MEMORY_UTILIZATION:-0.35}" \
  --kv-cache-memory-bytes "${KV_CACHE_MEMORY_BYTES:-2G}" --max-num-seqs "${MAX_NUM_SEQS:-2}" \
